...
\newcommand\pcode[5]{
	\begin{algorithm} \caption{#1} \label{pcode:#2} \begin{algorithmic}[1]
		\Function{#3}{#4} 
			#5
		\EndFunction
...
\pcode{Training Algorithm}{train}{train}{$config$}{
	...
	\For{$generation < config.maxGens$}
		...
		\While{$!pool.isEmpty()$}
			...
			\State i = i + 1
		\EndWhile
		\While{$survivors.size < config.population$}
			\State $survivors.add(offpring.poll().getNetwork())$
		\EndWhile
...
\subsection{Usage}
To create a classifier network, use $ClassificationGenticNeuralNetwork.train$ as shown below. Training configuration and hyperparameters are stored in a NeuralNetworkTrainingConfiguration object.
...
A regression network is created with $RegressionGenticNeuralNetwork.train$ instead, such as the example below which was trained on the $x^2$ function.
